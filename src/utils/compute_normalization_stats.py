...
def compute_means_stds_images(root_dir, train_csv, output_file_means="stats/means_summer_rgbnir.npy",
    ...
    if os.path.exists(output_file_means_path):
        ...
        for i, row in tqdm(df.iterrows()):
            ...
            stats_rows.append({'hotspot_id': hs, 'r': means[2], 'g': means[1], 'b': means[0], 'nir': means[3]})

        stats_df = pd.DataFrame(stats_rows, columns=["hotspot_id", "r", "g", "b", "nir"])
    ...
    if os.path.exists(output_file_stds_path):
        ...
        for i, row in tqdm(df.iterrows()):
            ...
            stats_rows_std.append({'hotspot_id': hs, 'r_std': std[2], 'g_std': std[1], 'b_std': std[0], 'nir_std': std[3]})

        stats_df_2 = pd.DataFrame(stats_rows_std, columns=["hotspot_id", "r_std", "g_std", "b_std", "nir_std"])
...
def compute_means_stds_images_visual(root_dir, train_csv, output_file_means="stats/means_summer_images_visual.npy",
    ...
    if os.path.exists(output_file_means_path):
        ...
        for i, row in tqdm(df.iterrows()):
            ...
            cropped = crop_center(arr, 64, 64)
            means = np.mean(np.mean(cropped, axis=0), axis=0)
            stats_rows.append({'hotspot_id': hs, 'r': means[0], 'g': means[1], 'b': means[2]})

        stats_df = pd.DataFrame(stats_rows, columns=["hotspot_id", "r", "g", "b"])
    ...
    if os.path.exists(output_file_stds_path):
        ...
        stats_rows_std: list[dict] = []
        for i, row in tqdm(df.iterrows()):
            hs = row["hotspot_id"]
            arr = tifffile.imread(os.path.join(root_dir, f"images_visual/{hs}_visual.tif"))
            cropped = crop_center(arr, 64, 64)
            std = ((cropped - means) ** 2 / (64 * 64)).sum(axis=0).sum(axis=0)
            stats_rows_std.append({'hotspot_id': hs, 'r_std': std[0], 'g_std': std[1], 'b_std': std[2]})

        stats_df_2 = pd.DataFrame(stats_rows_std, columns=["hotspot_id", "r_std", "g_std", "b_std"])
...
def compute_means_stds_env_vars(root_dir, train_csv, env, env_data_folder="environmental",
    ...
    if os.path.exists(output_file_means_path):
        ...
        for i, row in tqdm(df.iterrows()):
            ...
            stats_rows.append(per_raster_mean.tolist())

        stats_df = pd.DataFrame(stats_rows, columns=env_var_names)
    ...
    if os.path.exists(output_file_stds_path):
        ...
        stats_rows_std: list[list] = []
        for i, row in tqdm(df.iterrows()):
            hs = row["hotspot_id"]
            arr = np.load(os.path.join(root_dir, env_data_folder, f"{hs}.npy"))
            std = np.nansum(((arr - means[:, np.newaxis, np.newaxis]) ** 2) / (50 * 50), axis=-1)
            std = np.nansum(std, axis=-1)
            stats_rows_std.append(std.tolist())
        stats_df = pd.DataFrame(stats_rows_std, columns=env_var_names)
        stds_to_save = []
        for env_var in env_var_names:
