"""
全样本可解释性分析 V2 - 自动从checkpoint读取配置
"""
import sys
...
def infer_config_from_checkpoint(checkpoint_path):
    ...
    # 推断hidden_dim (从fusion.env_encoder.0.weight)
...
def load_model(checkpoint_path, device):
    ...
    model = Dinov2AdapterPrompt(
        ...
        env_hidden_dim=inferred['hidden_dim'],  # 关键
        use_env=True,
...
def analyze_domain(name, config, device):
    ...
    env_gradients = np.array(env_gradients).squeeze()
