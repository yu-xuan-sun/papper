...
from src.analysis.interpretability import InterpretabilityAnalyzer, InterpretabilityVisualizer
...
def load_dataset_simple(config: dict, base_dir: str, max_samples: int = None):
    import glob
    ...
    class SimpleDataset(Dataset):
        def __init__(self, df, data_dir, env_dim, num_species):
        ...
        def __getitem__(self, idx):
            try:
                ...
                if img.shape[0] >= 3:
                    ...
                    img_rgb = np.repeat(img[:1], 3, axis=0)
                
                # Resize到224x224
                from skimage.transform import resize
                img_resized = resize(img_rgb.transpose(1, 2, 0), (224, 224), preserve_range=True, anti_aliasing=True)
    ...
    dataset = SimpleDataset(df_test, data_dir, config['env_dim'], config['num_species'])
...
def run_analysis(domain: str, max_samples: int = 500, output_dir: str = 'outputs/interpretability'):
    ...
    model = load_model_direct(checkpoint_path, config['num_species'], config['env_dim'], device)
    
    print(f"\nLoading data from: {config['data_dir']}")
    dataloader = load_dataset_simple(config, base_dir, max_samples)
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        try:
            ...
            for i in range(images.shape[0]):
                ...
                gate_info = analyzer.analyze_gate_weights(img, env_vec)
                if gate_info and 'alpha' in gate_info:
                    all_alphas.append(gate_info['alpha'])
                
                env_importance = analyzer.compute_environmental_importance(img, env_vec)
                if env_importance is not None:
                    all_env_importances.append(env_importance)
    ...
    results = {
        ...
        'alpha_std': float(np.std(all_alphas)) if all_alphas else None,
        'env_dim': config['env_dim']
    }
    
    if all_env_importances:
        env_importance_mean = np.mean(all_env_importances, axis=0)
        results['env_importance'] = env_importance_mean.tolist()
        
        top_indices = np.argsort(env_importance_mean)[::-1][:5]
        results['top_env_indices'] = top_indices.tolist()
        results['top_env_values'] = env_importance_mean[top_indices].tolist()
    
    os.makedirs(output_dir, exist_ok=True)
    output_file = os.path.join(output_dir, f'{domain.replace("-", "_")}_real_analysis.json')
    ...
    if results.get('alpha_mean') is not None:
        print(f"  Alpha (visual weight): {results['alpha_mean']:.4f} ± {results['alpha_std']:.4f}")
        print(f"  Environmental weight: {1-results['alpha_mean']:.4f}")
    print(f"  Top 5 env var indices: {results.get('top_env_indices', 'N/A')}")
...
def main():
    parser = argparse.ArgumentParser(description='Run interpretability analysis on real data')
    ...
    for domain in domains:
        try:
            results = run_analysis(domain, args.max_samples, args.output_dir)
    ...
    if len(all_results) > 1:
        import json
        summary_file = os.path.join(args.output_dir, 'real_analysis_summary.json')
