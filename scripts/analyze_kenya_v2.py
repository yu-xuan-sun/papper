...
def load_sample(data_dir, hotspot_id, env_dim=19):
    ...
    if len(env_data.shape) > 1:
        if len(env_data.shape) == 3:
            env_data = env_data.mean(axis=(1, 2))
        else:
...
def load_kenya_model(ckpt_path, device='cuda'):
    ...
    print(f"  env_hidden_dim={ENV_HIDDEN_DIM}, env_dim={ENV_DIM}, num_classes={NUM_CLASSES}")
    ...
    for k in state_dict:
        if 'fusion.env_encoder.0.weight' in k:
            actual_env_dim = state_dict[k].shape[1]
            print(f"  Checkpoint env_dim: {actual_env_dim}")
        if 'fusion.gate.0.weight' in k:
            actual_hidden_dim = state_dict[k].shape[0]
            print(f"  Checkpoint hidden_dim: {actual_hidden_dim}")
...
def main():
    ...
    print(f"{'='*60}")
    print(f"Samples analyzed: {len(gate_values)}")
