...
def run_analysis(domain: str, max_samples: int = 500, output_dir: str = 'outputs/interpretability'):
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        try:
            ...
            if (batch_idx + 1) % 10 == 0:
                ...
                print(f"  Batch {batch_idx + 1}: {sample_count} samples analyzed, avg_alpha={alpha_mean:.4f}")
        except Exception as e:
    ...
    if all_env_importances:
        env_importances = np.array(all_env_importances)
        mean_importance = np.mean(env_importances, axis=0)
