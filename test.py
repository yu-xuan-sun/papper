...
def _find_latest_best_ckpt(save_root: str, run_name_prefix: str) -> str:
    """
    Search under save_root (usually runs/) for directories starting with run_name_prefix,
    sorted by name descending (timestamp suffix), and return checkpoints/best-*.ckpt if found.
    Fallback to checkpoints/last.ckpt. Return empty string if none.
    """
    ...
    try:
        ...
        candidates.sort(reverse=True)  # newest first by timestamp suffix
        for d in candidates:
            ckpt_dir = os.path.join(d, "checkpoints")
            if not os.path.isdir(ckpt_dir):
                continue
            best_list = sorted(glob.glob(os.path.join(ckpt_dir, "best-*.ckpt")), reverse=True)
            if best_list:
                return best_list[0]
...
def main(opts):
    ...
    if config.load_ckpt_path:
        ...
        # Auto-discover the most recent best checkpoint in runs/ to avoid evaluating a random model
        run_name = getattr(config.logging, "run_name", "default_run")
        run_id = args["run_id"]
        global_seed = get_seed(run_id, config.program.seed)
        run_prefix = f"{run_name}_seed{global_seed}_"
        runs_root = os.path.join(config.base_dir, config.save_path or "runs")
        auto_ckpt = _find_latest_best_ckpt(runs_root, run_prefix)
        if auto_ckpt:
            print(f"[Auto-CKPT] Using discovered checkpoint: {auto_ckpt}")
            # load checkpoint path relative to base_dir if needed
            if not os.path.isabs(auto_ckpt):
                auto_ckpt = os.path.join(config.base_dir, auto_ckpt)
            try:
                task = task.load_from_checkpoint(auto_ckpt, save_preds_path=config.save_preds_path)
            except Exception:
                # fallback: load only state_dict
                state = torch.load(auto_ckpt, map_location="cpu")
                task.load_state_dict(state.get("state_dict", state), strict=False)
            _ = test_task(task)
        else:
            print("No checkpoint provided and none discovered in runs/. Evaluating a random model.")
            _ = test_task(task)
